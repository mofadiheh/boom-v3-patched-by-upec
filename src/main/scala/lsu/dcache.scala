  ...
  with HasBoomCoreParameters
{
  ...
  val s2_nack_wb     = widthMap(w => s2_valid(w) && !s2_hit(w) && s2_wb_idx_matches(w))
	...
	//Delay-on-miss
	// nack if it is a speculative miss
	val s2_nack_speculative_miss = widthMap(w => s2_valid(w) && !s2_hit(w) && s2_req(w).is_speculative )
	//------------


  s2_nack           := widthMap(w => (s2_nack_miss(w) || s2_nack_hit(w) || s2_nack_victim(w) || s2_nack_data(w) || s2_nack_wb(w)) && s2_type =/= t_replay 
																	|| s2_nack_speculative_miss(w) /*Delay-on-miss*/)
  ...
  for (w <- 0 until memWidth) {
    mshrs.io.req(w).valid := s2_valid(w)          &&
														// Delay-on-miss
														!s2_req(w).is_speculative && // only send to mshr if it is a non-speculative miss
														//--------------
                            !s2_hit(w)            &&
                            !s2_nack_hit(w)       &&
                            !s2_nack_victim(w)    &&
