...
// Delay-on-miss
	val is_speculative = Bool() // Is it speculative, i.e., is there a chance that the instruction will be discarded by an earlier one
...
// Delay-on-miss
		dtlb.io.is_speculative(w) := false.B
		when (will_fire_load_incoming (w) || will_fire_stad_incoming (w) || will_fire_sta_incoming  (w)
			...
			dtlb.io.is_speculative(w) := (exe_tlb_uop(w).br_mask =/= 0.U) 
		}
// -------------
  }
...
// Delay-on-miss
    ...
    when (will_fire_load_incoming(w)) {
      ...
      dmem_req(w).bits.uop   := exe_tlb_uop(w)
			...
			// Delay-on-miss
    	dmem_req(w).bits.is_speculative := (exe_tlb_uop(w).br_mask =/= 0.U)
